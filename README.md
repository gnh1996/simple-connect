...
- **双栏 SFTP 浏览**：本地 | 远程双栏，Tab 切换焦点，支持目录递归的上传/下载/删除、单键/多选批量传输、路径传输与路径跳转（Tab 补全）
...
powershell -ExecutionPolicy Bypass -File "$env:TEMP\simple-connect-install.ps1" -Release v0.2.0
# 若你使用 PowerShell 7，最后一行可换为：
# pwsh -ExecutionPolicy Bypass -File "$env:TEMP\simple-connect-install.ps1" -Release v0.2.0
```
