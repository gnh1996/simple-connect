...
1. **shell 内注入（主路径，绕开 AcceptEnv）**：首次进入会话时经 `StdinPipe` 发送钩子命令（`cwdHookCommand`）：
   ...
   - 成功后仅发送一次（首次进入），SFTP 恢复时不发（原样恢复）。注入（`hookInjected`）与回显恢复（`echoRestored`）**各自记状态**：任一项写入失败时下次 Resume **只补发失败项**，已成功字节不重发（避免 `PROMPT_COMMAND` 重复追加）；清屏（`cleared`）单独记，不受注入结果影响。**回显隐藏靠 PTY 层 ECHO=0**（见 `ssh.Client.NewTerminalSession`）：pty-req 即关闭 ECHO；bash 的 readline 在 tty ECHO=0 时会关闭自身回显，注入命令不可见；**zsh 的 ZLE 始终重绘输入行，注入命令会在首屏被显示一次（纯观感，不影响功能）**。`stty echo` 由 `restoreRemoteEcho` **单独一行**紧随其后发送（恢复回显不依赖前一行能否被远程 shell 解析，见第 9 节）；命令本体**不含清行/清屏控制序列**（会干扰 bash readline 导致光标错位/输入不可见）；残余副作用：history 记录 1~2 条命令。
...
| 2026-08 | 注入后多一个空行（bash 在 ECHO=0 下处理输入行输出 `\x1b[?2004l\r\r\n`） | 注入命令被 bash 执行时会额外输出一行换行序列，屏幕出现孤立空行 | 曾追加 `clear`；后因 `clear` 的 terminfo 序列会清 scrollback 丢弃横幅/motd 而移除（首次清屏已由 `Handle.run` 的 `\x1b[2J\x1b[H` 完成） |
