...
1. **shell 内注入（主路径，绕开 AcceptEnv）**：首次进入会话时经 `StdinPipe` 发送钩子命令（`cwdHookCommand`）：
   - **共用**：先无条件定义 `_sc_cwd` 上报函数（检测 `${TMUX:-}`，tmux 内用 passthrough 序列 `\ePtmux;\e\e]133;cwd=%s\007\e\\`）；
   - **bash**：`export PROMPT_COMMAND="_sc_cwd${PROMPT_COMMAND:+; $PROMPT_COMMAND}"`——**追加保留用户已有钩子**（powerline 等），`${VAR:-}`/`:+` 展开保证 **`set -u`（nounset）安全**；
   - **zsh**：`eval 'precmd_functions+=(_sc_cwd)'`——`precmd_functions` **追加不覆盖**（兼容 oh-my-zsh 等）；zsh 专属数组语法经 `eval` 包裹，避免 POSIX sh 解析整行报错；
   - **sh/dash**：`${BASH_VERSION:-}`/`${ZSH_VERSION:-}` 均未定义，两分支短路静默（POSIX 无提示符前钩子机制，降级不跟踪）。
   - 成功后仅发送一次（首次进入），SFTP 恢复时不发（原样恢复）。注入（`hookInjected`）与回显恢复（`echoRestored`）**各自记状态**：任一项写入失败时下次 Resume **只补发失败项**，已成功字节不重发（避免 `PROMPT_COMMAND` 重复追加）；清屏（`cleared`）单独记，不受注入结果影响，且在请求 PTY 前完成（见第 1 节）。**回显隐藏靠 PTY 层 ECHO=0**（见 `ssh.Client.NewTerminalSession`）：pty-req 即关闭 ECHO；bash 的 readline 在 tty ECHO=0 时会关闭自身回显，注入命令不可见；**zsh 的 ZLE 始终重绘输入行，注入命令会在首屏被显示一次（纯观感，不影响功能）**——实测 zsh + oh-my-zsh 下钩子命令与 `stty echo` 共约 4 行文本（含逐字符高亮重绘）会永久留在首屏；彻底隐藏需 zsh shell integration（修改远程配置），当前不采用。「命令本体不含控制序列」的约束只针对**注入命令的内容**（避免干扰远程 readline）；本地清屏序列由 `clearScreen` 在会话输出开始前发出，不经过远程 stdin。`stty echo` 由 `restoreRemoteEcho` **单独一行**紧随其后发送（恢复回显不依赖前一行能否被远程 shell 解析，见第 9 节）；命令本体**不含清行/清屏控制序列**（会干扰 bash readline 导致光标错位/输入不可见）；残余副作用：history 记录 1~2 条命令。
...
- `TestSessionCwdHookInjected`：首次进入会话 shell 内注入 cwd 钩子（bash/zsh/tmux passthrough/set -u 安全追加），且 `stty echo` **独立成行**（非 POSIX shell 解析失败不致盲打）
- `TestCwdHookCommandNounsetSafe`：`set -u` + PROMPT_COMMAND/TMUX 未设置时钩子仍可安装并调用（需本机 bash，缺失则 skip）
- `TestCwdHookCommandZsh`：zsh 分支 `precmd_functions+=(_sc_cwd)` 在 `set -u` + 数组未设置时可用，且追加保留已有 `precmd_functions`（需本机 zsh，缺失则 skip）
- `TestInjectCwdHookWriteFailure`：钩子注入与回显恢复写失败时各自返回 false（调用方只重试失败项，避免 `PROMPT_COMMAND` 重复追加）
...
| 2026-08 | 注入命令回显难以隐藏（`stty -echo` 一次性写入靠时序赌注、慢网必漏；确认标记握手虽可靠但残留引导行） | 原实现 `inPipe.Write("stty -echo\r"+cwdHookCommand+"; stty echo\r")`；改确认标记握手后仍残留引导行 `stty -echo; printf ...` | **改 PTY 层 ECHO=0**（`ssh.Client.NewTerminalSession` pty-req 即关 ECHO）：bash 下注入命令完全**不回显、无任何残留**（zsh ZLE 仍会重绘，见第 3 节）；`stty echo` 恢复交互；`TestSessionPtyIUTF8` 断言 ECHO=0 回归 |
| 2026-08 | 注入后多一个空行（bash 在 ECHO=0 下处理输入行输出 `\x1b[?2004l\r\r\n`） | 注入命令被 bash 执行时会额外输出一行换行序列，屏幕出现孤立空行 | 曾追加 `clear`；后因 `clear` 的 terminfo 序列会清 scrollback 丢弃横幅/motd 而移除（首次清屏已由 `Handle.start` 的 `clearScreen` 在 PTY 请求前完成） |
| 2026-09 | 连接闪烁：远程登录 banner 一闪后整屏被清掉、随后约 0.6s 空白；zsh 下注入命令与 `stty echo` 共约 4 行留在首屏 | ① 首次清屏位于 `Handle.run()`（PTY 已建立、banner 已透传之后），擦掉的是 banner 而非 TUI 残留（TUI 为 alt-screen，退出时已自动复原），zsh + oh-my-zsh 初始化慢放大空白期；② pty ECHO=0 只对 bash readline 生效，ZLE 必然显示注入命令 | ① `clearScreen` 提前到 `Handle.start()` 的 `setupSession`（PTY 请求）之前，banner 完整保留；`cleared` 仍只置一次，Resume 不重复清屏；② 保持现状并如实文档化（彻底隐藏需 zsh shell integration，侵入远程配置，不采用）。`TestHandleClearBeforePtyRequest` / `TestHandleClearOnceAcrossResume` 回归 |
| 2026-09 | ① `set -u` 用户 cwd 钩子安装失败（unbound variable，跟踪静默失效）；② fish/csh 等非 POSIX shell 解析钩子行失败时，同行的 `stty echo` 被一并丢弃 → ECHO 永久关闭、输入不可见（盲打）；③ zsh 下注入命令被 ZLE 明文/重绘刷屏（文章宣称「完全不回显」失实）；④ 注入写入失败仍置 `started`，不重试 | ① 直接引用 `${PROMPT_COMMAND}`/`$ZSH_VERSION`/`$TMUX`，nounset 下报错；② `cwdHookCommand + "; stty echo\r"` 同一行，复杂命令解析失败会终止整行；③ readline 尊重 tty ECHO=0，ZLE 不尊重；④ 忽略 Write 错误且无条件置位 | ① 全部改 `${VAR:-}` / `${PROMPT_COMMAND:+...}`；② `stty echo` **独立成行**发送（`restoreRemoteEcho`）；③ 修正文档/注释，明示 zsh 首屏显示一次（观感问题），并精简合并 bash/zsh 共用函数；④ 注入（`injectCwdHook`）/回显恢复（`restoreRemoteEcho`）各自记状态（`hookInjected`/`echoRestored`），写失败时下次 Resume **只重试失败项**且落 `applog`；清屏单独记 `cleared`，不受注入结果影响。`TestCwdHookCommandNounsetSafe` / `TestCwdHookCommandZsh` / `TestInjectCwdHookWriteFailure` / `TestSessionCwdHookInjected` 回归 |
